import { Component, OnInit, EventEmitter, Output, Inject, ViewChild, TemplateRef, Renderer2, ElementRef, HostListener } from '@angular/core';
...
export class TopbarComponent implements OnInit {
  ...
  notifyId: any;
  headerBackgroundColor: string = '#e4eaf7';
  ...
  constructor(@Inject(DOCUMENT) private document: any, private eventService: EventService, public languageService: LanguageService, private modalService: NgbModal,
    ...
    private router: Router, private TokenStorageService: TokenStorageService, private renderer: Renderer2, private el: ElementRef) { }
  ...
  ngOnInit(): void {
    ...
    this.authFackservice.currentUserSubject.subscribe((res:any)=>{
    ...
    //     this.updateHeaderBackgroundColor(event.urlAfterRedirects);
    //   }
    // });
    // this.updateHeaderBackgroundColor(this.router.url);
  ...
  // for buyer login 
  // private updateHeaderBackgroundColor(url: string): void {
  //   switch (url) {
  //     case '/home':
  //       this.headerBackgroundColor = '#e4eaf7';
  //       break;
  //     default:
  //       this.headerBackgroundColor = '#ffffff'; // White background for other routes
  //       break;
  //   }
  // }
  ...
  openSignInModal() {
    ...
    modalRef.componentInstance.signInType = this.userType.BUYER
  ...
  buyerLoggin() {
    ...
    this.authFackservice.currentUserSubject.next(obj);
    location.reload()
  }
  ...
  openSellerSignIn() {
    const modalRef = this.modalService.open(SignInComponent, { size: 'xl', centered: true });
    modalRef.componentInstance.signInType = this.userType.SELLER;
    // modalRef.componentInstance.loginData = { email: 'ganesh@gmail.com', password: 'Ganesh@123' }
    // modalRef.result.then(() => {
    //   this.sellerLoggin();
    // }, () => { });
    modalRef.result.then(()=>{},()=>{})
  ...
  sellerLoggin() {
    ...
    localStorage.setItem('currentUser', JSON.stringify(obj));
    this.authFackservice.currentUserSubject.next(obj);
    this.router.navigate(['supplier-dashboard/dashboard']).then(()=>{
      location.reload()
    })
  }

  onDashboardSelect(url:string) {
    // if (this.currentUser.type === this.userType.SELLER) {
